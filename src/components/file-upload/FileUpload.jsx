...
const FileUpload = () => {
    const [uplodeFile, setUplodeFile] = useState(null);
    ...
    const rejectHandler = event => {
        ...
        // axios.delete(`http://45.139.10.161:8091/uploader/${fileName}`)
        //     .then((res) => {
    ...
    useEffect(() => {
        ...
        if (view) {
            ...
            axios
                .get(`http://45.139.10.161:8091/uploader/${fileName}/download/`)
                .then(res => {
                    imgView.src = `http://45.139.10.161:8091/uploader/${fileName}/download/`;
                })
                .catch(err => {
                    console.log(err);
                });
    ...
    const fileHandler = event => {
        ...
        //     imgInput.src = URL.createObjectURL(
        ...
        if (document.getElementById("file").files.length > 0) {
            if (label.trim().length != 0) {
                ...
                document.querySelector("#uploaded_files").style.top = "56px";
            }
        }
    ...
    const uploed = (start, uploedFile) => {
        ...
        const currentChunk = document
            .getElementById("file")
            .files[0].slice(start, nextChunk);
        const uploadedChunk = start + currentChunk.size;
        const finished =
            uploadedChunk >= document.getElementById("file").files[0].size
                ? 1
                : 0;
        ...
        const config = {
            onUploadProgress: function (progressEvent) {
                if (
                    document.getElementById("file").files[0].size < max_length
                ) {
                ...
                document.querySelector(".progress-bar").style.width =
                    percentCompleted + "%";
                document.querySelector(".progress-bar").style.height = "8px";
                console.log(document.querySelector(".progress-bar"));
                setIsLoading(true);
            },
        };
        const filename = document.getElementById("file").files[0].name;
        let textbox = "Uploading file";
        axios
            .post("http://45.139.10.161:8091/uploader/", formData, config)
            .then(res => {
                if (nextChunk < document.getElementById("file").files[0].size) {
                    ...
                    console.log("FILE ID: ", res.data.file_id);
    ...
    //     const x = <div>
    //         <div>
    //             <div className="rectionModal"></div>
    //             <div className="rejectionContainer">
    //                 <p className="textRejection">دلایل عدم تایید خود را وارد کنید</p>
    //                 <textarea
    ...
    //         <p>djk</p> */}
    //     </div>
    //     return setShowModalRejection(x)
    ...
    return (
        <div className="my-modal">
            <div className="modal-container">
                <form encType="multipart/form-data" action="">
                    {!view ? (
                                    ...
                                    {label.trim().length != 0 ? (
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                blurInput={onBlurInputValue}
                                                inputIsValid={validInputValue}
                                                label="شماره سفارش"
                                                isTouched={hasErrorInputValue}
                                                // required={required}
                                            />
                                        </div>
                                    ) : (
                        ...
                        <div>
                            ...
                            >
                                <div className="form-input">
                                    <div>
                                        ...
                                        {fileType == "payment" ? (
                                            <div className="divViewInfo">
                                                <p className="pViewInfo">
                                                    {/* {value} */}
                                                    name
                                                </p>
                                            </div>
                            ...
                            <img className="imgView" src="" alt="viewImg" />
                            {fileType == "payment" ? (
                                <button className="btnView">تایید</button>
                            ) : (
                                <div>
                                    <button className="unAcceptBtnView">
                                        عدم تایید
                                    </button>
                                    <button className="acceptBtnView">
                                        تایید
                                    </button>
                                    {showModalRejection}
                                </div>
                ...
                <div id="uploaded_files">{uplodeFile}</div>
            </div>
        </div>
    );
};
